...
/datum/game_mode/proc/try_latespawn(var/datum/mind/player, var/latejoin_only)
	...
	message_admins("[ruppertext(name)]: Attempting spawn.")
	...
	if(latejoin_only && latejoin_templates.len)
		...
		message_admins("[ruppertext(name)]: Failed to find configured mode spawn templates, please disable auto-antagonists until one is added.")
		round_autoantag = 0
		return
	...
	while(usable_templates.len)
		...
		if(spawn_antag.attempt_late_spawn(player))
			message_admins("[ruppertext(name)]: Attempting to latespawn [spawn_antag.id]. ([spawn_antag.get_antag_count()]/[spawn_antag.cur_max])")
